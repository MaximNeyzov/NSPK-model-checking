...
a_transfer_p == \* Передача сообщения от агента "a" к агенту "p".
    LET
        ...
        mes1 == <<[ key |-> id_to_pub[a_connect_to],
                    item |-> [ i1 |-> "Na",
                               i2 |-> "a" ] ]>>
        
        mes3 == <<[ key |-> id_to_pub[a_connect_to],
                    item |-> [ i1 |-> a_input_nonce,
                               i2 |-> "empty" ] ]>>
...
b_transfer_p == \* Передача сообщения от агента "b" к агенту "p".
    LET
        ...
        mes2 == <<[ key |-> id_to_pub[b_input_id],
                    item |-> [ i1 |-> b_input_nonce,
                               i2 |-> "Nb" ] ]>>
...
AcceptedMessages1 == \* Множество принимаемых агентом "b" сообщений типа mes1.
    LET
        Domain == \* Множество значений параметров, от которых зависит пораждаемое сообщение.
            (Nonces \ {"Nb"}) \X (ID \ {"b"})
...
\**********************************************************************
\* Множество принимаемых агентами сообщений данного типа:
...
\* Безопасная аутентификация в направлении от В к А. (not valid)
